...
void EvadeBehavior::update(Agent* owner, float deltaTime)
{
	...
	//Find the direction
	MathLibrary::Vector2 direction = MathLibrary::Vector2::normalize((owner->getWorldPosition() + owner->getVelocity()) - m_target->getWorldPosition());
...
void EvadeBehavior::draw(Agent* owner)
{
